...
<div class="relative">
  <button
    onclick={() => (open = !open)}
    class="flex items-center gap-1.5 rounded-lg bg-slate-800/90 backdrop-blur-sm px-2.5 py-1.5 text-xs text-slate-300 hover:text-white shadow-md ring-1 ring-white/10 transition-colors"
  ...
  {#if open}
    ...
    <ul
      class="absolute bottom-full mb-1 left-0 z-50 bg-slate-800 rounded-lg shadow-xl ring-1 ring-white/10 py-1 min-w-40"
      role="listbox"
      ...
      {#each BASEMAPS as basemap (basemap.id)}
        ...
        >
          {#if mapStore.basemapId === basemap.id}
            <svg class="h-3 w-3 text-blue-400 shrink-0" viewBox="0 0 12 12" fill="currentColor">
