<script lang="ts">
  ...
  function subscribeToProgress(id: string) {
    ...
    // Try SSE first
    const es = new EventSource(`/api/import/progress?jobId=${id}`);
    eventSource = es;
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'progress') {
          ...
          if (msg.layerId) onimported?.(msg.layerId);
          reset();
          open = false;
          ...
          clearPollIntervals();
          uploading = false;
          toastStore.error(msg.message ?? 'Import failed.');
        }
      } catch {
        // Parse error — ignore
      }
  ...
  async function pollJob() {
    ...
    if (pollStartedAt && Date.now() - pollStartedAt > POLL_TIMEOUT_MS) {
      ...
      uploading = false;
      toastStore.error(
        'Import is taking longer than expected. Check back later or try a smaller file.'
      );
      return;
    }
...
<Modal bind:open title="Import Data" dismissible={!uploading || pollTimedOut} onclose={reset}>
  ...
  {#if pollTimedOut}
    <!-- Timeout view -->
    <div
      class="flex flex-col items-center gap-4 py-6 bg-surface-container rounded-xl border border-white/5"
    >
      ...
      <p class="text-xs text-on-surface-variant text-center">
        Import is taking longer than expected. Check back later.
      </p>
    </div>
  {:else if uploading}
    <!-- Progress view -->
    <div class="flex flex-col gap-4">
      ...
      <div class="bg-surface-low rounded-lg border border-white/5 p-3 flex flex-col gap-2">
        <div class="flex items-center justify-between gap-2">
          <span class="text-xs font-semibold text-on-surface truncate flex-1"
            >{selectedFile?.name}</span
          >
          <span
            class="text-[9px] font-bold uppercase tracking-wider text-primary bg-primary/10 border border-primary/20 rounded px-2 py-0.5"
          >
  ...
  {:else if !selectedFile}
    ...
    >
      ...
      <p class="text-xs font-semibold text-on-surface mb-1">
        Drop your file here or click to browse
      </p>
      <p class="text-xs text-on-surface-variant">
        GeoJSON, CSV, KML, GPX, GeoPackage, and Shapefiles (zipped)
      </p>
      <p class="text-[10px] text-on-surface-variant mt-2">
        Imported files become new layers on your map.
      </p>
    </div>
    <input
      id="file-input"
  ...
  {:else}
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex items-center gap-3 bg-surface-low rounded-lg border border-white/5 p-3">
        <div
          class="flex items-center justify-center w-8 h-8 rounded bg-primary/10 border border-primary/20 flex-shrink-0"
        >
          <svg
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[9px] font-mono text-on-surface-variant uppercase tracking-wider">
  ...
  {#snippet footer()}
    ...
    {:else if !uploading && selectedFile}
      ...
      <Button variant="primary" onclick={startUpload}>Import</Button>
    {/if}
  {/snippet}
</Modal>
