<script lang="ts">
  ...
  const cancelMutation = createMutation(() => ({
    mutationFn: (jobId: string) => trpc.geoprocessing.cancel.mutate({ jobId }),
    onSuccess: () => {
      running = false;
      cancelling = false;
  ...
  const OP_ICONS: Record<OpType, typeof CircleDot> = {
    buffer: CircleDot,
    convex_hull: Triangle,
    centroid: Crosshair,
    dissolve: Droplets,
    intersect: Merge,
    union: CircleDashed,
    clip: Scissors,
    point_in_polygon: MapPin,
  ...
  function subscribeToProgress(jobId: string) {
    ...
    try {
      const es = new EventSource(`/api/import/progress?jobId=${encodeURIComponent(jobId)}`);
      eventSource = es;
      ...
      es.onmessage = (event) => {
        try {
          ...
          if (data.type === 'progress') {
            ...
            progress = 100;
            success = `Created layer "${data.layerId ? 'new layer' : 'output'}"`;
            closeEventSource();
            queryClient.invalidateQueries({ queryKey: queryKeys.layers.list({ mapId }) });
            onlayercreated(data.layerId ?? '');
            running = false;
  ...
  function startPolling(jobId: string) {
    ...
    pollInterval = setInterval(async () => {
      try {
        const res = await fetch(`/api/import/progress?jobId=${encodeURIComponent(jobId)}`);
        if (!res.ok) {
        ...
        // Poll endpoint returns SSE-formatted lines; parse the last data: line
    ...
    switch (opType) {
      ...
      case 'dissolve':
        return {
          type: 'dissolve',
          layerId: layerIdA,
          ...(dissolveField.trim() ? { field: dissolveField.trim() } : {}),
        };
      case 'intersect':
  ...
  function extractErrorMessage(err: unknown, op: OpType): string {
    ...
    const hints: Partial<Record<OpType, string>> = {
      ...
      point_in_polygon:
        'Point-in-polygon failed — ensure the points layer has point geometries and the polygons layer has polygon geometries.',
    };
    return (
      hints[op] ?? `${GEO_OP_LABELS[op]} operation failed. Please check your inputs and try again.`
    );
  }
  ...
  async function handleRun(e: Event) {
    ...
    try {
      ...
      if (!abortController.signal.aborted) {
        // Subscribe to SSE progress for the async job
...
<div
  class="flex flex-col h-full {embedded !== true
    ? 'bg-surface-container border-l border-white/5'
    : ''}"
>
  {#if embedded !== true}
    <!-- Header -->
  ...
  {#if layers.length === 0}
    <div class="flex flex-col items-center justify-center py-12 px-4 text-center flex-1">
      ...
      <p class="text-sm text-on-surface-variant">
        Add a layer with features first, then run spatial operations here.
      </p>
    </div>
  {:else}
    <!-- Form -->
      ...
      <div class="grid grid-cols-4 gap-1.5">
        {#each OP_TYPES as op (op)}
          ...
          >
            ...
            <span class="text-[8px] font-bold uppercase tracking-wider mt-1 leading-tight"
              >{GEO_OP_LABELS[op]}</span
            >
          </button>
        {/each}
      </div>
      ...
      {#if running && currentJobId}
        <div class="flex flex-col gap-1">
          <div class="flex justify-between text-[10px] text-on-surface-variant">
            <span>{GEO_OP_LABELS[opType]} in progress…</span>
      ...
      {#if !running}
        ...
        {:else if TWO_LAYER_OPS.has(opType) && !layerIdB}
          <p class="text-xs text-on-surface-variant/70">
            Select a second layer for this operation.
          </p>
        {:else if AGG_OPS.has(opType) && aggregation !== 'count' && !aggField.trim()}
          <p class="text-xs text-on-surface-variant/70">
            Enter a numeric field name for {aggregation} aggregation.
          </p>
        {/if}
      {/if}
    </form>
