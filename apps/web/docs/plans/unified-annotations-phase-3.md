...
- **A.4** — `onfeaturedrawn` chain: trace MapEditor consumer; it likely refetches features. Decide: (a) keep the callback firing for backward-compat (parent re-fetch is wasted but harmless); (b) replace with `onannotationdrawn` and update MapEditor; (c) remove entirely and let annotation cache invalidation handle parent updates. **Default: (b)** — revised 2026-04-25 after pre-work trace of `handleFeatureDrawn` in `MapEditor.svelte:262-290`. The handler does THREE things, only one of which annotation cache-invalidation already covers:
  1. **`loadLayerData(layerId)` re-fetch** — covered by `createAnnotationMutationOptions.onSuccess` invalidating `queryKeys.annotations.list({mapId})`.
  2. **`transitionTo({type: 'featureSelected', feature: {featureId, layerId, geometry}})`** — load-bearing UX: this is what selects the just-drawn shape so the user sees handles + can immediately edit/style. NOT covered by cache invalidation. Removing the callback (option c) drops the post-draw selection feedback.
  3. **`activityStore.log('feature.drawn', {layerId, layerName, geometryType})`** — observability event; needs an annotation-equivalent (`activityStore.log('annotation.drawn', ...)`).
  Therefore A.4 must replace the callback (option b), passing the new annotation's `id` + `anchor` so MapEditor can transition to an `annotationSelected` (or equivalent) state and log the corresponding event. Plain removal would silently regress draw-then-select UX.
