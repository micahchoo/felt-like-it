...
docker compose -f "$SCRIPT_DIR/docker-compose.yml" stop web worker martin || true
...
gunzip -c "$DB_FILE" | docker compose -f "$SCRIPT_DIR/docker-compose.yml" exec -T postgres \
...
if [ -f "$UPLOADS_FILE" ]; then
  ...
  docker compose -f "$SCRIPT_DIR/docker-compose.yml" run --rm -T \
    -v felt-like-it_uploads:/uploads \
    --entrypoint "" \
